...
public class Tablero {
	...
	public void atacar(int fila, int columna, int fila2, int columna2, Equipo e) {
		...
		if(((filaInfEnRango(fila, columna, fila2) || filaSupEnRango(fila, columna, fila2)) &&
				(!colInfEnRango(fila, columna, columna2) && !colSupEnRango(fila, columna, columna2))) 
			|| ((colInfEnRango(fila, columna, columna2) || colSupEnRango(fila, columna, columna2)) &&
					(!filaInfEnRango(fila, columna, fila2) && !filaSupEnRango(fila, columna, fila2))))

			System.out.println("El personaje seleccionado no está en el radio de ataque");


		else if ((colInfEnRango(fila, columna, columna2) || colSupEnRango(fila, columna, columna2)) &&
				(filaInfEnRango(fila, columna, fila2) || filaSupEnRango(fila, columna, fila2))) {
	...
	public boolean filaInfEnRango(int fila, int columna, int fila2) {
		if(fila2<(fila - tablero[fila][columna].getRadioAtaque()))
			return false;
		return true;
	}

	public boolean filaSupEnRango(int fila, int columna, int fila2) {
		if(fila2>(fila + tablero[fila][columna].getRadioAtaque()))
			return false;
		return true;
	}

	public boolean colInfEnRango(int fila, int columna, int columna2) {
		if(columna2<(fila - tablero[fila][columna].getRadioAtaque()))
			return false;
		return true;
	}

	public boolean colSupEnRango(int fila, int columna, int columna2) {
		if(columna2>(fila + tablero[fila][columna].getRadioAtaque()))
			return false;
		return true;
	}
